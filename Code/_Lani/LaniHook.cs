...
{   
    class LaniHook : Entity
    {
        ...
        float max_length_cur, max_length;
        float state_timer, koef_sinusoidal, length, length_grab, hook_speed, hook_cooldown;
        DateTime dt_grab;
        ...
        public LaniHook(Player player, Vector2 direction, LaniHookParams hook_params)
        {
            ...
            this.hook_speed = hook_params.Speed;
            this.hook_cooldown = hook_params.Cooldown;
            this.koef_sinusoidal = 0;
            this.state = LaniEnum_State.Start;
            this.state_grab = LaniEnum_Grab.Nothing;
            this.color = hook_params.Color;  //Color.Pink * 0.77f; // Color.Blue * 0.5f

            //Color.Pink;      // 255, 192, 203
            // ColorPink 77% = // 196, 148, 156

            Audio.Play(SFX.char_mad_dash_red_left);
            Depth = (MeliHelperModule.Settings.Debug_LaniHookShowAhhhh) ? -9999999 : (player.Depth - 1);
        }
        ...
        public override void Added(Scene scene)
        {
            ...
            state_grab = CheckCollision(player.Center + direction, true, ref grab_solid, ref grab_entity, ref grab_object);
            if (state_grab != LaniEnum_Grab.Nothing)
                loc_start = player.Center;
        }
        ...
        public override void Update()
        {
            ...
            //Logger.Log("LaniHook", $"state={state};  grab={state_grab};  timer={state_timer};  player.Center={player.Center};  player.Speed={player.Speed}");
            //Logger.Log("LaniHook", $"state={state};  grab={state_grab};  player.Speed={player.Speed.Length()};  X={player.Speed.X};  Y={player.Speed.Y}");
            ...
            switch (state)
            {
                case LaniEnum_State.Start:
                    if (Input.Dash.Check && max_length_cur < max_length)
                        max_length_cur = (int)Math.Min(max_length, length + 16);
                    length += hook_speed * Engine.DeltaTime;
                    loc_end = loc_start + direction * length;
                    player.Center = loc_start - hook_origin;
                    player.Speed = Vector2.Zero;

                    state_grab = CheckCollision(loc_end, false, ref grab_solid, ref grab_entity, ref grab_object);
                ...
                case LaniEnum_State.MovePlayer:
                    ...
                    loc_start += dir_speed * hook_speed * Engine.DeltaTime;
                    length = Vector2.Distance(loc_end, loc_start);
                    ...
                    if (length >= 10)
                    {
                        grab_error = CheckCollision(loc_start, true, ref grab_solid_fly, ref grab_entity, ref grab_object);
                        if (grab_error == LaniEnum_Grab.Nothing)
                            grab_error = CheckCollision(loc_start + new Vector2(0, 5), true, ref grab_solid_fly, ref grab_entity, ref grab_object);
                        if (grab_error == LaniEnum_Grab.Nothing)
                            grab_error = CheckCollision(loc_start - new Vector2(0, 5), true, ref grab_solid_fly, ref grab_entity, ref grab_object);
                    }
                    ...
                    else if (Input.Jump.Check && !is_player_already_jumped)
                    {
                        // Jump!
                        Vector2 player_speed = new Vector2(
                            0.85f * dir_speed.X * hook_speed, 
                            Math.Min(-200, dir_speed.Y * hook_speed - 240)
                            );
                    ...
                    else if (!Input.Dash.Check)
                    {
                        ...
                        player.Speed = dir_speed * hook_speed;
                        SetStateRelease(0.2f);
                    }
                    else
                    {
                    ...
                    switch (state_grab)
                    {
                        ...
                        case LaniEnum_Grab.GrabObject:
                            if (grab_object == null || grab_object.GetHoldable == null || !Input.Dash.Check)
                            {
                                ...
                                RemoveSelf();   // UPD: SetStateRelease instead, maybe?
                                return;
                            }

                            loc_start = player.Center + hook_origin;

                            int millis = (int)(DateTime.Now - dt_grab).TotalMilliseconds;
                            float attract_koef = (millis < 100) ? 0.2f :
                                                 (millis > 400) ? 0.8f :
                                                 (0.2f + 0.6f * (millis - 100) / 300);
                        ...
                        default:
                            ...
                            state_timer -= Engine.DeltaTime;
                            hook_cooldown -= Engine.DeltaTime;
                            if (state_timer <= 0)
                            {
                                if (hook_cooldown <= 0)
                                    RemoveSelf();
                            ...
                            if (state_grab == LaniEnum_Grab.HookAir && Input.Jump.Check && !is_player_already_jumped)
                            {
                                // Jump!
        ...
        void SetStateRelease(float timer = 0f)
        {
            ...
            state_timer = timer; //(hook_cooldown > timer) ? hook_cooldown : timer;
            player.RefillDash();
        ...
        void ActivateBlockAfterHookCollision(Solid solid)
        {
            ...
            if (grab_solid is ZipMover)
                InteractionController.ActivateBlockZipMover(grab_solid as ZipMover, player);

            if (grab_solid is MoveBlock)
                InteractionController.ActivateBlockMove(grab_solid as MoveBlock, player);

            if (grab_solid is SwapBlock)
                InteractionController.ActivateBlockTempleSwap(grab_solid as SwapBlock, player);
        ...
        void ActivateBlockAfterPlayerCollision(Solid solid)
        {
            if (solid != null)
            {
                if (solid is DashBlock)
                    InteractionController.ActivateBlockDash(solid as DashBlock, player);

                if (solid is FallingBlock)
                    InteractionController.ActivateBlockFalling(solid as FallingBlock, player);

                if (solid is TempleCrackedBlock)
                    InteractionController.ActivateBlockTempleCracked(solid as TempleCrackedBlock, player);
                ...
                if (solid is LightningBreakerBox)
                    InteractionController.ActivateBlockFarewellLightningBreaker(solid as LightningBreakerBox, player, Vector2.Zero);
        ...
        LaniEnum_Grab CheckCollision(Vector2 loc, bool is_check_solids_only, 
            ref Solid solid, ref Entity entity, ref LaniThrowableObject obj)
        {
            ...
            if (solid != null)
            {
                ...
                {
                    Vector2 loc_dist = new Vector2(0, 3);
            ...
            //                                                                                            // length 40 -> 120   ==>   radius 8 -> 12 
            entity = level.Entities.FindAll<LaniHookSpot>().FirstOrDefault(t => Vector2.Distance(loc, t.Center) <= t.GetRadius + Math.Max(0, (length - 40) / 20));
        ...
        public override void Render()
        {
            ...
            for (int i = 0; i < dist; i++)
            {
                ...
                Draw.Rect(loc, 1, 1, color);
                Draw.Rect(loc - perp, 1, 1, Color.White);
            }

            if (MeliHelperModule.Settings.Debug_LaniHookShowAhhhh)
            {
            ...
            //    Methods.DrawText(player.Center + new Vector2(0, 24), "cooldown: " + hook_cooldown.ToString(), Color.White);
            //}
        }
    }
}
