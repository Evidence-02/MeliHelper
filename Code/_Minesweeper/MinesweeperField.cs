...
namespace Celeste.Mod.MeliHelper._Minesweeper
{
    ...
    class MinesweeperField : Entity
    {
        ...
        int w, h, count_bombs, count_empty_cells, max_start_cell, seed;
        string diagonal_dashes;
        bool is_place_player_inside, is_autogenerated;
        ...
        public override void Added(Scene scene)
        {
            ...
            if (is_autogenerated)
            {
                // Generate correct map field
                char[,] map;
                if (seed == 0)
                {
                    int counter = 18;   // 18 attempts isn't a joke
                    do
                    {
                        seed = ++MeliHelperModule.Instance.SaveData.MinesweeperFieldSeed;
                        map = MinesweeperFieldGenerator.GenerateFieldFromSeed(w, h, count_bombs, seed);
                    }
                    while (!MinesweeperFieldGenerator.isAppropriateField(w, h, map) && --counter > 0);
                }
                else
                    map = MinesweeperFieldGenerator.GenerateFieldFromSeed(w, h, count_bombs, seed);
                ...
                for (int i = 0; i < w; i++)
                    for (int j = 0; j < h; j++)
                        level.Add(mass_cells[i, j] = new MinesweeperCell(this,
                            position: Position + 16 * new Vector2(i, j),
                            is_bomb: map[i, j] == 'B',
                            count_neighbors: map[i, j] - '0'));
            }
        }
        ...
        public override void Awake(Scene scene)
        {
            ...
            // Find cell with 0 or 1 neighbors (or more if needed), open it and then "player.Center = cell.Position;"
            if (is_place_player_inside && Methods.PlayerIsAlive(player))
            {
                ...
                {
                    Calc.PushRandom(MeliHelperModule.Instance.SaveData.MinesweeperFieldSeed);
        ...
        public bool TryToRegisterCellThere(MinesweeperCell cell)
        {
            int cx = (int)(cell.Position.X - Position.X) / 16;
            int cy = (int)(cell.Position.Y - Position.Y) / 16;
            if (isInField(cx, cy))
            {
                mass_cells[cx, cy] = cell;
                return true;
            }
            return false;
        }
        ...
        public void RegisterOpenedCell()
        {
            ...
            if (count_empty_cells <= 0 && !MeliHelperModule.Settings.DebugTools.MinesweeperSolvedFromStart)
            {
                ...
                if (teleport_info.GetRoomName != "")
                    SceneAs<Level>().Add(new CutsceneRoomTeleport(teleport_info));
            }
        }
